      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">{event.clientName} - {event.venue}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge status={event.status} />
            {event.isPinnedForMe && <StatusBadge status="Pinned" />}
            {canPin && <button onClick={pinEvent} className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold hover:bg-slate-50"><Pin size={16} /> {event.isPinnedForMe ? 'Unpin' : 'Pin main'}</button>}
            {canManage && <button onClick={() => setEditingEvent(!editingEvent)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold hover:bg-slate-50">Edit</button>}
            {isBoss(user) && <button onClick={deleteEvent} className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white">Delete</button>}
          </div>
        </div>
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
          {tabs.map(([key, label, Icon]) => (
            <button key={key} onClick={() => setTab(key)} className={`flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold ${tab === key ? 'bg-green-50 text-brand' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
              <Icon size={16} /> {label}
            </button>
          ))}
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p className="text-sm text-slate-500">Pending / revision amount</p><p className="mt-2 text-2xl font-bold">Rs {totalSubmitted}</p></div>
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><p className="text-sm text-slate-500">Approved / paid amount</p><p className="mt-2 text-2xl font-bold">Rs {totalApproved}</p></div>
      </div>
      {!canReview && (
        <form onSubmit={submit} className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-3">
          ...
          <div className="flex gap-2">
            <button className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">{editing ? 'Update expense' : 'Submit expense'}</button>
            {editing && <button type="button" onClick={() => { setEditing(null); setForm(empty); }} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold">Cancel</button>}
          </div>
        </form>
      )}
        ...
        {items.map((item) => (
          <div key={item._id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {!canReview && String(item.submittedBy?._id || item.submittedBy) === String(user._id) && ['Draft', 'Submitted', 'Revision Required', 'Rejected'].includes(item.status) && <button onClick={() => startEdit(item)} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-brand">Edit</button>}
              {!canReview && String(item.submittedBy?._id || item.submittedBy) === String(user._id) && ['Draft', 'Submitted', 'Revision Required', 'Rejected'].includes(item.status) && <button onClick={() => remove(item)} className="rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-semibold text-rose-600">Delete</button>}
              {canReview && <button onClick={() => review(item, 'Approved')} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-white">Approve</button>}
              {canReview && <button onClick={() => review(item, 'Revision Required')} className="rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white">Revision</button>}
              {canReview && <button onClick={() => review(item, 'Rejected')} className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white">Reject</button>}
              {canReview && ['Approved'].includes(item.status) && <button onClick={() => review(item, 'Paid')} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-white">Mark paid</button>}
            </div>
          </div>
        ))}
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm shadow-sm">
      ...
      <div className="mt-3 border-t border-slate-100 pt-3">
        ...
        <form onSubmit={addComment} className="mt-3 flex gap-2"><input value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Add task comment" /><button className="rounded-lg bg-brand px-3 py-2 text-xs font-semibold text-white">Add</button></form>
      </div>
    </div>
  );
}
...
function Checklist({ items, team, eventId, canManage, reload }) {
  ...
  return (
    <div className="space-y-4">
      {canManage && <form onSubmit={submit} className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-4"><select value={form.section} onChange={(e) => setForm({ ...form, section: e.target.value })}>{['Before Event', 'During Event', 'After Event'].map((s) => <option key={s}>{s}</option>)}</select><input placeholder="Checklist item" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><select value={form.assignedTo} onChange={(e) => setForm({ ...form, assignedTo: e.target.value })} required><option value="">Assign to</option>{team.map((u) => <option key={u._id} value={u._id}>{u.fullName}</option>)}</select><button className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">Add item</button></form>}
      <div className="grid gap-3 lg:grid-cols-3">{items.map((item) => <div key={item._id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><button onClick={() => toggle(item)} className="block w-full text-left hover:text-brand"><p className="text-xs font-semibold text-slate-500">{item.section}</p><p className={`mt-1 text-sm font-medium ${item.completed ? 'line-through text-slate-400' : ''}`}>{item.title}</p><p className="text-xs text-slate-500">{item.assignedTo?.fullName}</p></button>{canManage && <button onClick={() => remove(item._id)} className="mt-3 flex items-center gap-1 text-xs font-semibold text-rose-600"><Trash2 size={13} /> Delete</button>}</div>)}</div>
    </div>
  );
}
...
function Notes({ items, eventId, canManage, reload }) {
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={submit} className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-2"><input placeholder="Note title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required /><select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>{['General', 'Entry Gate', 'Parking', 'Emergency', 'Schedule Change'].map((t) => <option key={t}>{t}</option>)}</select><textarea placeholder="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><button className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">Add note</button></form>
      <div className="grid gap-3 lg:grid-cols-2">{items.map((note) => <div key={note._id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><div className="flex justify-between gap-3"><h3 className="font-semibold">{note.title}</h3><div className="flex items-center gap-2"><StatusBadge status={note.type} />{canManage && <button onClick={() => remove(note._id)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50" title="Delete"><Trash2 size={16} /></button>}</div></div><p className="mt-2 text-sm text-slate-600">{note.description}</p><p className="mt-2 text-xs text-slate-500">By {note.createdBy?.fullName}</p></div>)}</div>
    </div>
  );
}
...
function Submissions({ items, tasks, eventId, user, canReview, canManage, reload }) {
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={submit} className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-4"><select value={form.taskId} onChange={(e) => setForm({ ...form, taskId: e.target.value })} required><option value="">Select task</option>{tasks.map((task) => <option key={task._id} value={task._id}>{task.title}</option>)}</select><input placeholder="Submission note" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /><input type="file" multiple onChange={(e) => setForm({ ...form, files: [...e.target.files] })} /><button className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">Submit work</button></form>
      <div className="grid gap-3 lg:grid-cols-2">{items.map((item) => <div key={item._id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><div className="flex justify-between gap-3"><h3 className="font-semibold">{item.taskId?.title}</h3><StatusBadge status={item.status} /></div><p className="mt-2 text-sm text-slate-600">{item.message}</p>{item.files?.length > 0 && <div className="mt-2 text-sm">{item.files.map((file) => <a key={file.path} href={fileUrl(file)} target="_blank" rel="noreferrer" className="block text-brand hover:underline">{file.filename}</a>)}</div>}<p className="text-xs text-slate-500">By {item.submittedBy?.fullName}</p>{canReview && <div className="mt-3 flex flex-wrap gap-2"><button onClick={() => review(item._id, 'Approved')} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-white">Approve</button><button onClick={() => review(item._id, 'Revision Required')} className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white">Reject</button>{canManage && <button onClick={() => remove(item._id)} className="flex items-center gap-1 rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-semibold text-rose-600"><Trash2 size={13} /> Delete</button>}</div>}</div>)}</div>
    </div>
  );
}
