...
export function EventCard({ event, onPin, canPin = false }) {
  return (
    <article className={`rounded-2xl border bg-white p-4 shadow-sm sm:p-5 ${event.isPinnedForMe ? 'border-brand ring-2 ring-green-100' : 'border-slate-200'}`}>
      <Link to={`/events/${event._id}`} className="block hover:text-brand">
        <div className="space-y-2">
          ...
          <h3 className="min-w-0 break-words text-lg font-black leading-6 text-slate-950">{event.eventName}</h3>
        </div>
      ...
      {canPin && <div className="mt-4 border-t border-slate-100 pt-3">
        <button onClick={() => onPin?.(event)} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold hover:bg-slate-50 sm:w-auto">
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {['Pending', 'In Progress', 'Submitted', 'Approved', 'Revision Required'].map((status) => (
          <button key={status} onClick={() => onUpdate(task._id, { status })} className="min-h-10 rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold hover:bg-slate-50">{status}</button>
        ))}
        {canManage && <button onClick={() => onDelete?.(task._id)} className="flex min-h-10 items-center justify-center gap-1 rounded-xl border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50"><Trash2 size={13} /> Delete</button>}
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        {['Assigned', 'Packed', 'Brought to Event', 'Missing', 'Returned'].map((status) => (
          <button key={status} onClick={() => onUpdate(item._id, { status })} className="min-h-10 rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold hover:bg-slate-50">{status}</button>
        ))}
        {canManage && <button onClick={() => onDelete?.(item._id)} className="flex min-h-10 items-center justify-center gap-1 rounded-xl border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50"><Trash2 size={13} /> Delete</button>}
      </div>
    </div>
  );
}
